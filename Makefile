...
SOURCE_FILES := $(shell find . -name "*.c" | grep -v Project_Windows) 
...
CC := avr-gcc
CFLAGS := -std=c11 -mmcu=$(TARGET_CPU) -ggdb -Wall 
...
$(BUILD_DIR):
	for line in `find . -name "*.c"`; do mkdir -p $(BUILD_DIR)/`dirname $$line`; done;
